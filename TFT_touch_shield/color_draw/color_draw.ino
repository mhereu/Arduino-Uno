...
char *name = "Catalina";  //edit name of shield
const int XP=8,XM=A2,YP=A3,YM=9; //320x480 ID=0x9486
const int TS_LEFT=110,TS_RT=912,TS_TOP=946,TS_BOT=90;
...
void setup(void)
{
    ...
    tft.fillRect(BOXSIZE * 5, 0, BOXSIZE, BOXSIZE, BROWN);

    tft.drawRect(0, 0, BOXSIZE, BOXSIZE, WHITE);
    currentcolor = RED;
    tft.drawRect(tft.width() - tft.width()/8, tft.height() - 40, tft.width(), tft.height(), WHITE);
}
    ...
    if (tp.z > MINPRESSURE && tp.z < MAXPRESSURE) {
        ...
        if (ypos < BOXSIZE) {               //draw white border on selected color box
            ...
            if (oldcolor != currentcolor) { //rub out the previous white border
                ...
                if (oldcolor == BROWN) tft.fillRect(BOXSIZE * 5, 0, BOXSIZE, BOXSIZE, BROWN);
            }
        }
        ...
        if (xpos > tft.width() - tft.width()/8 && ypos > tft.height() - 40) {
